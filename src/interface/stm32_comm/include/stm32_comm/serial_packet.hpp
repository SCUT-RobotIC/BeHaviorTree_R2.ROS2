...
// ========================================
#ifndef STM32_COMM__SERIAL_PACKET_HPP_
#define STM32_COMM__SERIAL_PACKET_HPP_

#include "stm32_comm/common_headers.hpp"
#include "stm32_comm/action_codes.hpp"
#include "stm32_comm/utils.hpp"
#include <sstream>
#include <iomanip>
...
inline void serialize_packet(const SerialPacket& packet, std::vector<uint8_t>& buffer) {
  ...
  buffer[12] = packet.stag_x & 0xFF;
  buffer[13] = (packet.stag_x >> 8) & 0xFF;
  buffer[14] = packet.stag_y & 0xFF;
  buffer[15] = (packet.stag_y >> 8) & 0xFF;
  buffer[16] = packet.stag_detected & 0xFF;
  buffer[17] = (packet.stag_detected >> 8) & 0xFF;
  buffer[18] = packet.arm_joint1 & 0xFF;
  buffer[19] = (packet.arm_joint1 >> 8) & 0xFF;
  buffer[20] = packet.arm_joint2 & 0xFF;
  buffer[21] = (packet.arm_joint2 >> 8) & 0xFF;
  buffer[22] = packet.arm_joint3 & 0xFF;
  buffer[23] = (packet.arm_joint3 >> 8) & 0xFF;
  buffer[24] = packet.arm_yaw & 0xFF;
  buffer[25] = (packet.arm_yaw >> 8) & 0xFF;
  buffer[26] = packet.x_offset & 0xFF;
  buffer[27] = (packet.x_offset >> 8) & 0xFF;
  buffer[28] = packet.y_offset & 0xFF;
  buffer[29] = (packet.y_offset >> 8) & 0xFF;

  for (size_t i = 0; i < 32; ++i) {
    buffer[30 + i] = packet.reserved[i];
  }
...
inline SerialPacket deserialize_packet(const std::vector<uint8_t>& data_buffer) {
  ...
  packet.stag_x = utils::combine_bytes<int16_t>(&data_buffer[14]);
  packet.stag_y = utils::combine_bytes<int16_t>(&data_buffer[16]);
  packet.stag_detected = utils::combine_bytes<int16_t>(&data_buffer[18]);
  packet.arm_joint1 = utils::combine_bytes<int16_t>(&data_buffer[20]);
  packet.arm_joint2 = utils::combine_bytes<int16_t>(&data_buffer[22]);
  packet.arm_joint3 = utils::combine_bytes<int16_t>(&data_buffer[24]);
  packet.arm_yaw = utils::combine_bytes<int16_t>(&data_buffer[26]);
  packet.x_offset = utils::combine_bytes<int16_t>(&data_buffer[28]);
  packet.y_offset = utils::combine_bytes<int16_t>(&data_buffer[30]);

  for (size_t i = 0; i < 32; ++i) {
    packet.reserved[i] = data_buffer[32 + i];
  }
...
#endif // STM32_COMM__SERIAL_PACKET_HPP_
