...
from perception.pose_est.learning.models.refine_network import RefineNet
...
class PoseRefinePredictor:
  def __init__(self,):
    ...
    # 尝试使用相对路径，如果不存在则使用绝对路径
    weights_dir = f'{code_dir}/../../weights/{self.run_name}'
    if not os.path.exists(weights_dir):
      # 在install目录运行时，使用源代码的绝对路径
      weights_dir = f'/home/gzic-robotic/rc2026_ros2_ws/src/perception/pose_est/weights/{self.run_name}'
    
    ckpt_dir = f'{weights_dir}/{model_name}'
    self.cfg = OmegaConf.load(f'{weights_dir}/config.yml')
